...
class _TableHeaderDelegate extends SliverPersistentHeaderDelegate {
  ...
  @override
  double get minExtent => isSmallScreen ? 100 : 120;

  @override
  double get maxExtent => isSmallScreen ? 100 : 120;
...
class _GroupViewPageState extends State<GroupViewPage>
  ...
  void initState() {
    ...
    _tableScrollController.addListener(_syncScroll);
    _playersScrollController.addListener(_syncPlayersScroll);
  ...
  void _syncScroll() {
    if (_playersScrollController.hasClients &&
        _tableScrollController.hasClients) {
      _playersScrollController.jumpTo(_tableScrollController.offset);
    }
  }

  void _syncPlayersScroll() {
    if (_tableScrollController.hasClients &&
        _playersScrollController.hasClients) {
      _tableScrollController.jumpTo(_playersScrollController.offset);
    }
  }
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Container(
                          width: UI.isSmallScreen(context) ? 160 : 250,
                          decoration: BoxDecoration(
                            color: UI.card,
                            border: const Border(
                              right: BorderSide(color: UI.border, width: 1),
                            ),
                          ),
                          child: SingleChildScrollView(
                        ...
                        // Прокручиваемая область с тренировками
                        Expanded(
                          child: SingleChildScrollView(
                            scrollDirection: Axis.horizontal,
                            child: SingleChildScrollView(
                              ...
                              child: Column(
                                children: [
                                  // Заголовок таблицы (только тренировки)
                                  Container(
                                    height: UI.isSmallScreen(context)
                                        ? 100
                                        : 120,
                                  ...
                                  // Строки игроков (только тренировки)
                                  ...players.asMap().entries.map((entry) {
                                    final index = entry.key;
                                    final player = entry.value;
